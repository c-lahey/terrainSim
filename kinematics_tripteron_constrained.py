...
class TripteronConstrained:
    """
    Constraint-based Tripteron model from tripteron_extracted_model.json.

    This version is actuator-driven first:
      forward(sA, sB, sC) -> EE center + geometry

    and also supports:
      inverse(x, y, z) -> actuator values + geometry

    using a single consistent multibody-style solve.
    """
    ...
    def __init__(self, model_path: str | Path = "tripteron_extracted_model.json"):
        ...
        # Last state for continuation
        self._last_forward_state = np.hstack([
            self.ee_center_ref,
            self.legA.slider_value_ref,
            self.legB.slider_value_ref,
            self.legC.slider_value_ref,
            self._to2d(self.legA.elbow_ref, self.legA.plane),
            self._to2d(self.legB.elbow_ref, self.legB.plane),
            self._to2d(self.legC.elbow1_ref, self.legC.plane),
            self._to2d(self.legC.elbow2_ref, self.legC.plane),
        ])
    ...
    # Residual assembly
    # ---------------------------------------------------------
    def _state_unpack(self, x: np.ndarray):
        """
        x =
          ee_x, ee_y, ee_z,
          sA, sB, sC,
        ...
        C1el = x[10:12]
        C2el = x[12:14]
        return ee, sA, sB, sC, Ael, Bel, C1el, C2el

    def _residuals(self, x: np.ndarray, mode: str, target=None):
        ee, sA, sB, sC, Ael2, Bel2, C1el2, C2el2 = self._state_unpack(x)
        ...
        # Base joints ride with the same translating carriage/body
        ...
        # ---- Closed leg C upper branch
        r.append(norm(C1_el - C_base1) - self.legC.L1a)
        r.append(norm(C_ee1 - C1_el) - self.legC.L2a)

        # ---- Closed leg C lower branch
        r.append(norm(C2_el - C_base2) - self.legC.L1b)
        r.append(norm(C_ee2 - C2_el) - self.legC.L2b)

        # Mild regularization toward CAD posture / continuity
        Ael_ref2 = self._to2d(self.legA.elbow_ref, self.legA.plane)
        Bel_ref2 = self._to2d(self.legB.elbow_ref, self.legB.plane)
        C1el_ref2 = self._to2d(self.legC.elbow1_ref, self.legC.plane)
        C2el_ref2 = self._to2d(self.legC.elbow2_ref, self.legC.plane)

        r.extend(1e-3 * (Ael2 - Ael_ref2))
        r.extend(1e-3 * (Bel2 - Bel_ref2))
        r.extend(1e-3 * (C1el2 - C1el_ref2))
        r.extend(1e-3 * (C2el2 - C2el_ref2))

        # Solve mode constraints
        if mode == "forward":
            sA_t, sB_t, sC_t = target
            r.append(1e2 * (sA - sA_t))
            r.append(1e2 * (sB - sB_t))
            r.append(1e2 * (sC - sC_t))
        elif mode == "inverse":
            ee_t = np.asarray(target, dtype=float)
            r.extend(1e2 * (ee - ee_t))
    ...
    # ---------------------------------------------------------
    def forward(self, sA: float, sB: float, sC: float):
        """
        Actuator-driven solve.
        Inputs are prismatic joint positions.
        Output is EE center + full geometry.
        """
        x0 = self._last_forward_state.copy()
        x0[3:6] = [sA, sB, sC]

        sol = least_squares(
            lambda x: self._residuals(x, mode="forward", target=(sA, sB, sC)),
            x0,
            method="trf",
            max_nfev=200,
        )

        if (not sol.success) or np.linalg.norm(sol.fun) > 1e-3:
            raise TripteronKinematicsError("Forward solve failed.")

        self._last_forward_state = sol.x.copy()
        return self._geometry_from_state(sol.x)

    def inverse(self, x: float, y: float, z: float):
        """
        EE-driven solve.
        Input is EE center.
        Output is actuator positions + full geometry.
        """
        x0 = self._last_forward_state.copy()
        x0[0:3] = [x, y, z]

        sol = least_squares(
            lambda X: self._residuals(X, mode="inverse", target=(x, y, z)),
            x0,
            method="trf",
            max_nfev=200,
        )
        ...
        if (not sol.success) or np.linalg.norm(sol.fun) > 1e-3:
            raise TripteronKinematicsError("Inverse solve failed.")
    ...
    # Geometry packaging
    # ---------------------------------------------------------
    def _geometry_from_state(self, x: np.ndarray):
        ee, sA, sB, sC, Ael2, Bel2, C1el2, C2el2 = self._state_unpack(x)
        ...
        return {
            ...
            "A": {"slider": A_slider, "base": A_base, "elbow": A_el, "ee": A_ee},
            "B": {"slider": B_slider, "base": B_base, "elbow": B_el, "ee": B_ee},
            "C": {
                "slider": C_slider,
                "base1": C_base1,
                "elbow1": C1_el,
